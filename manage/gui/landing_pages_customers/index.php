...
$query="SELECT ".$update_table.".*, (SELECT COUNT(wm_pages.ID) FROM wm_pages WHERE wm_landing_pages_customers=wm_landing_pages_customers.ID AND Page_Type=56) AS numPages, (SELECT COUNT(wm_pages.ID) FROM wm_pages WHERE wm_landing_pages_customers=wm_landing_pages_customers.ID AND Page_Type=57) AS numElements   
	FROM ".$update_table."  
	ORDER BY ID";
...
<?php echo $arr[$i]["numPages"];?> דפים
/
<?php echo $arr[$i]["numElements"];?> אלמנטים
/
&nbsp;<?php echo file_exists("../../webfiles/landing_pages_customers/".$arr[$i]["ID"])?niceSize(getsize("../../webfiles/landing_pages_customers/".$arr[$i]["ID"]."/")):"0KB";?>&nbsp;נפח
			</div>
		</div>
