...
if($_REQUEST["import"] && $_REQUEST["table"] && is_uploaded_file($_FILES["fileToImport"]["tmp_name"])){
	...
	$filePath="../../".$cfg["WM"]["File_Uploades_Folder"]."/import/";
	...
	$dbFields=array("Email", "Name", "Enabled");
	
	$affectedRows=$im->importData($fullFilePath, $_POST["table"], $dbFields);
}
...
if($_POST){
	if($affectedRows){?>
	<div>ייבוא מוצלח של <?php echo $affectedRows;?> רשומות</div>
	<?php }else{?>
