...
if(isset($_REQUEST['selectedHomePage'])){
	$where = "homePageId=".intval($_REQUEST['selectedHomePage']);
}else{
	$where='1';
}

$query="SELECT ID, Name, Hide_On_Menu, File_Name FROM ".$update_table." WHERE ".$where;
...
/*get array of links for each homepage*/
$homePages = $db->getArray("SELECT ID,Name FROM wm_pages WHERE Page_Type = 5");
foreach ($homePages as $key => $value) {
	$arrLinks[$value['ID']]=$db->getArray("SELECT * FROM wm_links_header WHERE homePageId = ".$value['ID']);
}
...
function reloadPage(){
	var selected = $('#selectedHomePage').attr('value');
	location.href = location.href + "&selectedHomePage="+selected;
...
<select id="selectedHomePage" onchange="reloadPage()">
	<option><?php echo $trans->getText('select')?></option>
	<?php 
	foreach ($homePages as $key => $value) {?>
	...
	<div class="listItemImageContent">
		...
		<div class="listItemImageImage">
			<?php if($arr[$i]["File_Name"] && file_exists("../../".$arr[$i]["File_Name"])){?>
				<img style="width: 50px;height: 50px;" src="<?php echo $cfg["WM"]["Server"]."/".$arr[$i]["File_Name"];?>" border="1" />
			<?php };?>
		</div>
