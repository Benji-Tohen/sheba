...
if($_POST){
	if($id){
		...
		$qn->editQuestionair($id, $values);
	}else{
		$qn->addQuestionnaire($_POST["Name"]);
		
		header("location: index.php?show=questionair/index");
		exit;
	}
}
...
<a style="color: #ffffff;" href="index.php?show=questionair/index"><?php echo $text["Questionair"];?></a> -> <?php echo $row?$row["tree_name"]:$text["Add Item"];?>
</div>
...
<input type="hidden" name="show" value="questionair/edit" />
