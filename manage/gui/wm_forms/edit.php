<?php
require_once('../../classes/file/class.file.php');
require_once('../../classes/elad/elad_integration.class.php');
require_once('data.php');

$id=$_REQUEST["id"];
$page_id=$_REQUEST["page_id"];

$content_update=new ContentUpdater($db, $update_table);
/*
$arrTypes=array(
"0"     =>      "N/A",
"1"     =>      "Event",
"2"     =>      "Show",
"3"     =>      "Donation" 
);
*/

$arrTypes=$db->getArray("SELECT * FROM wm_event_types ORDER BY Value");

if($_POST){
    



        $check_inputs = array(
            array("string255"   => $_POST["Name"]),
            array("text"        => $_POST["Value"]),
            array("text"        => $_POST["send_mail_to"]),
            array("number"      => $_POST["wm_landing_pages_customers"]),
            array("number"      => $_POST["EventType_ID"]),

            array("text"        => $_POST["meta_title"]),
            array("text"        => $_POST["meta_desc"]),
            array("text"        => $_POST["meta_canonical"]),
            array("text"        => $_POST["meta_alternate"]),
            array("text"        => $_POST["h1"]),
            array("number"      => $_POST["meta_index"])
        );
        
        $secureTexts = new secure_inputs();
        $error = $secureTexts->isNotSecure($check_inputs);


        if (!$error) {

            $_SESSION["Translate"]["refresh"]=true;


            $arrFields=array(
                        "Name"				 =>	$_POST["Name"],
                        "Value"				 =>	$_POST["Value"],
                        "send_mail_to"                   =>     $_POST["send_mail_to"],
                        "wm_landing_pages_customers"	 =>	$_POST["wm_landing_pages_customers"],
                        "EventType_ID"	                 =>	$_POST["EventType_ID"],

                        "meta_title"    =>      $_POST["meta_title"],
                        "meta_desc"     =>      $_POST["meta_desc"],
                        "meta_canonical"        =>      $_POST["meta_canonical"],
                        "meta_alternate"        =>      $_POST["meta_alternate"],
                        "h1"    =>      $_POST["h1"],
                        "meta_index"    =>      ($_POST["meta_index"]?1:0)

        );



    /*
            if(!$id){
                    $query="SELECT MAX(Ordering) AS max_ordering FROM ".$update_table." WHERE WM_Pages=".$_POST["page_id"];
                    $max_ordering=$db->getField($query, "max_ordering");

                    $ordering=$max_ordering+1;

                    $arrFields["Ordering"]=$ordering;
            }
    */	

            $new_id=$content_update->update($id, $arrFields);
            if(!$id){
                    $id=$new_id;
                   }
                $formSent=true;


    /*
            if(is_uploaded_file($_FILES['user_file']['tmp_name'])){
                    $file_path="webfiles/".$folderName."/".$id."/";
                    $full_file_path="../../".$file_path; 
                    @chmod($full_file_path, 0777);

                    $file=new File();
                    $file->checkPath($full_file_path);

                    $file_name=str_replace(" ", "_", $_FILES['user_file']['name']);

                    $content_update->update($id, array("File_Name"=>$file_path.$file_name));
                    move_uploaded_file($_FILES['user_file']['tmp_name'], $full_file_path.$file_name);
                    @chmod($full_file_path.$file_name, 0777);
            }
    */
            if(!$_POST["SubmitAdd"]){
                    header("location: index.php?show=".$folderName."/index&page_id=".$page_id."&search=".$_REQUEST["search"]);
                    exit;
            }else{
                    $id=NULL;
            }
        }
}

if($id && $id>0){
	$row_item=$content_update->getValues($id);
	
}


$gui=new Gui("he");
?>
<?php require_once('common/header.php');?>
<script language="javascript" type="text/javascript">
function chooseFile(filePath){
	document.edit.Code.value=filePath;
}
</script>
<?php require_once('common/body.php');?>
<div class="navigator_line">
<a style="color: #ffffff;" href="index.php?show=wm_forms/index"><?php echo $text['all forms'];?></a>
<a style="color: #ffffff;" href="index.php?show=<?php echo $folderName;?>/index"><?php echo $pageName;?></a> -> <?php echo $row_item?$row_item["Name"]:$text["Add Item"];?>
</div>

<div class="editPagePadding">
<div class="SecurityMessage"><?php echo $error;?></div>
<br /><br />
<form action="index.php" name="edit" method="post" enctype="multipart/form-data">
<input type="hidden" name="show" value="<?php echo $folderName;?>/edit" />
<input type="hidden" name="id" value="<?php echo $row_item["ID"];?>" />
<input type="hidden" name="page_id" value="<?php echo $page_id;?>" />
<input type="hidden" name="search" value="<?php echo $_REQUEST["search"];?>" />
<table>		
	<tr>
		<td colspan="2">
		<?php if($row_item["File_Name"] && file_exists("../../".$row_item["File_Name"])){?>
			
<?php 
$fileNameArr=$row_item["File_Name"];
list($name, $ext)=explode("[.]", $fileNameArr);
?>
<?php if($ext=="swf"){?>
<object width="<?php echo $row_item["width"];?>" height="<?php echo $row_item["height"];?>"><param name="movie" value="<?php echo $cfg["WM"]["Server"]."/".$row_item["File_Name"];?>"></param><param name="wmode" value="transparent"></param><embed src="<?php echo $cfg["WM"]["Server"]."/".$row_item["File_Name"];?>" type="application/x-shockwave-flash" wmode="transparent" width="<?php echo $row_item["width"];?>" height="<?php echo $row_item["height"];?>"></embed></object>
<?php }elseif($row_item["File_Name"]){?>			
<img src="<?php echo "../../".$row_item["File_Name"];?>" width="<?php echo $row_item["width"];?>" height="<?php echo $row_item["height"];?>" />
<?php }?>


		<?php };?>
		</td>
	</tr>
	<tr>
		<td><?php echo $text["Name"];?>:</td>
		<td><input type="text" name="Name" value="<?php echo $row_item["Name"];?>" /></td>		
	</tr>
	<tr>
		<td valign="top"><?php echo $text["Value"];?>:</td>
		<td>
			<textarea name="Value" style="width: 200px; height: 100px;"><?php echo $row_item["Value"];?></textarea>
		</td>		
	</tr>
	<?php /*<tr>
		<td valign="top"><?php echo $text["Send Mail To"];?>:</td>
		<td>
			<textarea name="send_mail_to" style="width: 200px; height: 100px;"><?php echo $row_item["send_mail_to"];?></textarea>
		</td>		
	</tr>
    */?>

	<tr>
		<td valign="top">Type ID:</td>
		<td>
<select name="EventType_ID">
<?php foreach($arrTypes as $key=>$val){?>
	<option value="<?php echo $val["Value"];?>" <?php echo $val["Value"]==$row_item["EventType_ID"]?"selected":"";?>><?php echo $val["Name"];?></option>
<?php }?>
</select>
		</td>		
	</tr>

	<tr>
		<td valign="top">Customer:</td>
		<td>
<?php 
$query="
SELECT * 
FROM wm_landing_pages_customers 
ORDER BY Name
";
$arrForms=$db->getArray($query);
?>
<select name="wm_landing_pages_customers">
	<option value="0">-- NO CUSTOMER --</option>
<?php foreach($arrForms as $form){?>
	<option value="<?php echo $form["ID"];?>" <?php echo $form["ID"]==$row_item["wm_landing_pages_customers"]?"selected":"";?>><?php echo $form["Name"];?></option>
<?php }?>
</select>
		</td>		
	</tr>


	<tr>
		<td valign="top"><?php echo $text["meta_title"];?>:</td>
		<td><input type="text" name="meta_title" value="<?php echo $row_item["meta_title"];?>" /></td>
	</tr>
	<tr>
		<td valign="top"><?php echo $text["meta_desc"];?>:</td>
		<td>
			<textarea name="meta_desc" style="width: 200px; height: 100px;"><?php echo $row_item["meta_desc"];?></textarea>
		</td>		
	</tr>
	<tr>
		<td valign="top"><?php echo $text["meta_canonical"];?>:</td>
		<td>
			<textarea name="meta_canonical" style="width: 200px; height: 100px;"><?php echo $row_item["meta_canonical"];?></textarea>
		</td>		
	</tr>        
	<tr>
		<td valign="top"><?php echo $text["meta_alternate"];?>:</td>
		<td>
			<textarea name="meta_alternate" style="width: 200px; height: 100px;"><?php echo $row_item["meta_alternate"];?></textarea>
		</td>		
	</tr>
	<tr>
		<td valign="top"><?php echo $text["h1"];?>:</td>
                <td><input type="text" name="h1" value="<?php echo $row_item["h1"];?>" /></td>		
	</tr>        
	<tr>
		<td valign="top"><?php echo $text["meta_index"];?>:</td>
		<td>
			<input type="checkbox" name="meta_index" value="1" <?php echo $row_item["meta_index"]?"checked":"";?> />
		</td>		
	</tr>
	<tr>
		<td colspan="2" align="<?php echo $gui->getRight();?>"><input type="Submit" name="SubmitAdd" value="<?php echo $text["Update And Add"];?>" /><input type="Submit" name="Submit" value="<?php echo $text["Update"];?>" /></td>		
	</tr>		
</table>
</form>
</div>
<script language="javascript" type="text/javascript">
document.edit.Name.focus();
</script>
