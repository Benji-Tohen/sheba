...
$numItemsPerPage=50;//$params->getValue("pages_list_num_items_per_page");
...
<script type="text/javascript">
  ...
  $( "#duplicateFormInput" ).mouseleave(function() {
          var Name=$('#duplicateFormInput').val();

          $.ajax({
            url: '<?php echo $cfg["WM"]["Server"];?>/manage/gui/page_types/check_fileName_exist.php?checkName='+Name,
            success: function (data) {
                if(Name!==""){
                    if(data=='ok'){
                        $('#takenError').css('color','rgb(17, 230, 17)');
                        $('#takenError').html(data);
                    }else{
                        $('#takenError').css('color','red');
                        $('#takenError').html(data);
                    }
  ...
  function duplicateMe(url){

      var theData="url="+url;
      $.ajax({
          type: "POST",
          data: theData,
          url: "<?php echo $cfg["WM"]["Server"];?>/manage/gui/page_types/open_duplicate_form.php",
          success: function( data ) {
              $("#duplicateAjaxFormArea").append(data);
          }
      });
...
<li id="listItem_<?php echo $arr[$i]["ID"];?>" class="listItem_<?php echo ($i%2==0?0:1);?>">
	...
	<div class="listItemContent">
        ...
        <div class="listItemIcon">
            <a href="#" onclick="duplicateMe('/manage/gui/page_types/duplicate_page_type.php?id=<?php echo $arr[$i]["ID"];?>&amp;Name=<?php echo $arr[$i]["Name"];?>');"><img border="0" src="images/icons/duplicate.png" alt="Edit" /></a>
        </div>
        <div class="itemSap"></div>
...
<?php }?>
    ...
    <div id="duplicateAjaxFormArea"></div>
