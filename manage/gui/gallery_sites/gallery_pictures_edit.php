...
if($_POST){
	...
	$file_path=$cfg["WM"]["File_Uploades_Folder"]."GallerySites/".$page_id."/".$id."/";
	...
	if(!$_POST["SubmitAdd"]){
		header("location: index.php?show=gallery_sites/gallery_pictures&page_id=".$page_id);
		exit;
	}else{
		$id=NULL;
	}
...
<a style="color: #ffffff;" href="index.php?id=<?php echo $page_row["Parent"];?>&show=gallery_sites/gallery_pictures"><?php echo $text["Gellery"];?></a> ->
<?php }?>
<a style="color: #ffffff;" href="index.php?page_id=<?php echo $page_id;?>&show=gallery_sites/gallery_pictures"><?php echo $row_page["Name"];?></a> -> <?php echo $row_picture?$row_picture["Name"]:$text["Add Item"];?>
</div>
...
<input type="hidden" name="show" value="gallery_sites/gallery_pictures_edit" />
...
<table>		
	...
	<tr>
		...
		<td><input type="text" name="Code" value="<?php echo $row_picture["Code"];?>" dir="ltr" /> <a style="color: #ffffff !important;" href="#" onclick="window.open('gallery_sites/browse.php',
