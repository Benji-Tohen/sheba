...
if(strcmp($_POST["do"], "duplicate")==0){
	...
	foreach($arr as $item){
		...
		$newId=$db->updateData("wm_css_settings", $item, NULL);
		...
		foreach($arrInner as $inner){
			...
			$newInnerId=$db->updateData("wm_css_settings", $inner, NULL);
		}
...
$query="SELECT ID, Name, Closed FROM ".$update_table." WHERE wm_template=".$template;

if($_REQUEST["search"]){
	$query.=" AND (Name LIKE '%".$_REQUEST["search"]."%' OR class_name LIKE '%".$_REQUEST["search"]."%'  OR color LIKE '%".$_REQUEST["search"]."%' 
OR background_color LIKE '%".$_REQUEST["search"]."%'
OR border LIKE '%".$_REQUEST["search"]."%'
OR border_color LIKE '%".$_REQUEST["search"]."%'
OR font_size LIKE '%".$_REQUEST["search"]."%'
) 
";
}else{
...
<li id="listItem_<?php echo $arr[$i]["ID"];?>" class="listItem_<?php echo ($i%2==0?0:1);?>" style="<?php echo $arr[$i]["Closed"]?"background-color: #ff0000;":"";?>">
	...
	<div class="listItemContent">
		<div class="listItemText" dir="<?php echo $gui->getDir();?>">
			<a href="index.php?show=<?php echo $folderName;?>/index&amp;parent=<?php echo $arr[$i]["ID"];?>"><?php echo $arr[$i]["Name"];?></a>
		</div>
