...
$gui=new Gui("he");
if(isset($_REQUEST['Parent_id'])){/*need to show children of parent*/
    $parent = " AND Parent=".intval($_REQUEST['Parent_id']);
}else{
    $parent = " AND Parent=0";
}
$query="SELECT ID, Name, Hide_On_Menu, File_Name FROM ".$update_table." WHERE Domain_Parent=$domain_parent AND 1";
...
<div class="itemsListPageHeader">
	<a href="index.php?show=<?php echo $folderName;?>/edit&amp;page_id=<?php echo $page_id;?>&amp;Parent_id=<?php echo  isset($_REQUEST['Parent_id']) ? intval($_REQUEST['Parent_id']) : 0?>"><img border="0" src="images/icons/add_Page01.png" style="cursor: pointer; float: left;" alt="Add" /></a>
	...
	<div class="listItemImageContent">
		...
		<div class="listItemImageImage">
			<?php if($arr[$i]["File_Name"] && file_exists("../../".$arr[$i]["File_Name"])){?>
                            <img width="50" src="<?php echo $cfg["WM"]["Server"]."/".$arr[$i]["File_Name"];?>" border="1" />
			<?php };?>
		</div>

                <div style="cursor: pointer;width: 500px;" onclick="changeLayer(this); top.main1.location='index.php?show=side_menu_links/index&Parent_id=<?php echo $arr[$i]["ID"];?>'" class="listItemText" dir="<?php echo $gui->getDir();?>">
			<?php echo $arr[$i]["Name"];?>
		</div>
		...
		<div class="listItemIcon">
			<a href="index.php?show=<?php echo $folderName;?>/edit&amp;page_id=<?php echo $page_id;?>&amp;id=<?php echo $arr[$i]["ID"];?>&amp;search=<?php echo $_REQUEST["search"];?>&amp;Parent_id=<?php echo  isset($_REQUEST['Parent_id']) ? intval($_REQUEST['Parent_id']) : 0?>"><img border="0" src="images/icons/Edit01.png" alt="Edit" /></a>
		</div>
		<div class="itemSap"></div>
