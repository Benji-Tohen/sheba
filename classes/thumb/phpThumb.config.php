...
$serverRootDirectory=str_replace("classes/thumb/phpThumb.php", "", $_SERVER["SCRIPT_FILENAME"]);
//echo getenv('DOCUMENT_ROOT')."webfiles/cache/";
...
$PHPTHUMB_CONFIG['cache_directory'] = $serverRootDirectory."/webfiles/phpcache/"; //dirname(__FILE__).'/cache/';                            // set the cache directory relative to the phpThumb() installation
...
$PHPTHUMB_CONFIG['cache_source_directory'] = $serverRootDirectory."/webfiles/phpcache/source/";//dirname(__FILE__).'/cache/source/';  // set the cache directory for unprocessed source images
...
$PHPTHUMB_CONFIG['nooffsitelink_valid_domains'] = array(@$_SERVER['SERVER_NAME']);              // This is the list of domains for which thumbnails are allowed to be created. The default value of the current domain should be fine in most cases, but if neccesary you can add more domains in here, in the format 'www.example.com'
...
$PHPTHUMB_CONFIG['nooffsitelink_text_message']  = 'Image taken from '.@$_SERVER['SERVER_NAME']; // text of error message (used if [nooffsitelink_watermark_src] is not a valid image)
