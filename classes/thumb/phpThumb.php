...
if (@$_GET['src'] && !@$PHPTHUMB_CONFIG['allow_local_http_src'] && preg_match('/^http\:\/\/'.@$_SERVER['SERVER_NAME'].'(.+)/i', @$_GET['src'], $matches)) {
...
/*
list($nothing, $cacheFile)=explode("webfiles", $phpThumb->cache_filename);

$cacheFile="http://".$_SERVER["HTTP_HOST"]."/webfiles".$cacheFile;

//$redirectLine="\r\nRedirect 301 ".$_SERVER["REQUEST_URI"]." ".$cacheFile;

//$redirectLine="\r\nRewriteCond %{QUERY_STRING} id=1";
//$redirectLine.="RewriteRule ^sub-dir/index\.php$ /".$phpThumb->cache_filename."/? [L,R=301]";

//RewriteCond %{QUERY_STRING} ^zc=(.*)&w=(.*)$&h=(.*)&src=(.*)
//RewriteRule ^(.*)$ http://kgh.co.il/$4

file_put_contents(".htaccess", $redirectLine, FILE_APPEND);
*/
