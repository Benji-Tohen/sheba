...
$drExpertise = $db->getRow("SELECT wm_doctor_expertise.$doctorExpertiseInLang  FROM wm_pages INNER JOIN wm_doctor_expertise ON wm_doctor_expertise.ID = wm_pages.wm_doctor_expertise WHERE wm_pages.ID = ".$id);


if(!strpos($_SERVER['HTTP_REFERER'],'sheba.co.il')){
	$backLink = "window.location.href ={$wmPage['Parent']}";
}else{
	$backLink = 'history.back()';
}

$drExpertise=array('Name'=>$trans->getText('Areas of Expertise'),'Value'=>($drExpertise[$doctorExpertiseInLang]));
$arrDynamicFieldsSecondBlock[]=$drExpertise;
$connectedInstitutes = $wm->getConnectedPages($id,"95","Ordering");
$drInstitues = array();
$drInstitues['Value']='';
