...
$cachefile = $_SERVER['DOCUMENT_ROOT'].'/webfiles/html/menu/menuCache_'.$_SERVER['SERVER_NAME'].'.html';
...
<nav class="navbar navbar-default" role="navigation" >
    <div class="container">
	    <div class="navbar-header">
	        ...
	        <div title="<?php echo $trans->getText("Main Bar");?>" class="sideMenuToggle hidden-sm hidden-md hidden-lg ts">
            ...
            <form id='searchMobileForm' class="searchMobileForm" role="search" action="<?php echo $cfg["WM"]["Server"].'/item/'.$wm->getIdByPageTypeNoHomepageID(12,$wmPage['Lang']);?>" method="get">
                <div class="input-group">
                    <input type="text" name="q" class="form-control searchBox" placeholder="<?php echo $trans->getText("headerSearchTextMsr");?>" value="" />
                    <input type="hidden" name="search" value="1" />
                    <span onclick="$('#searchMobileForm').submit()" onkeypress="$('#searchMobileForm').submit()" class="input-group-addon searchButton"><i class="fa fa-search"></i></span>
                </div>
            </form>
        </div>
	    <!--/.navbar-header-->
	    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav" style="float: none;">
                    <button title="<?php echo $trans->getText("Main Bar");?>" class="sideMenuToggle hidden-xs ts"><img src="<?php echo $cfg["WM"]["Server"];?>/site/images/info.svg" alt="<?php echo $trans->getText("Main Bar");?>" title="<?php echo $trans->getText("Main Bar");?>" class="svg" /></button>
                                        ...
                                        foreach($arrTopSubMenu as $key=> $dir){
                                            ...
                                            <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3">
                                                <ul class="multi-column-dropdown">
                                                    ...
                                                    ?>
                                                        <li><a href="<?php echo $link;?>" target="<?php echo $target_child;?>"><?php echo $dirChild["Name"];?></a></li>
                                                    <?php }?>
                                                </ul>
                                            </div>
                                        <?php } ?>
