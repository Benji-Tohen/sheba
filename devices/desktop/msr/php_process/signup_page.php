...
/*
$arrChangedPages=$db->getArray("select distinct (wm_pages) from wm_pages_versions where Start_Date >= '2016-03-03'");

$db_array = array();
foreach ($arrChangedPages as $key ) {
	$row = $db->getRow("select * from wm_pages where id=".$key["wm_pages"]);
	array_push($db_array,$row);
			
}
$db_array=json_encode($db_array);
file_put_contents("english_db.sql",print_r($db_array,1));
*/
