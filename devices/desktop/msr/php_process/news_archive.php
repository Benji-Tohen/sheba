...
//$arr=	$sp->getPage($page);
if((devMode && $wm->get($homePageId,"domain")=="eng.shebatest.co.il")|| $wm->get($homePageId,"domain")=="eng.sheba.co.il"){
	$sp=	$wm->getNewsListPager($wmPage["ID"],"Start_Date DESC, Start_Time DESC, Name", $numItems);
	$arr=	$sp->getPage($page);
}else{
	$arr = $wm->getConnectedPages($wmPage["ID"],103,"wm_pages.Start_Date DESC","0,10");
}
